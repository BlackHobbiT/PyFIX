...
def main():
    ...
    group.add_argument('-s', '--sessions', dest='sessions', nargs='+', action='store', metavar=('s1', 's2'),
                       help='session to examine')
    parser.add_argument('filename', action='store', help='filename of the store file')
    parser.add_argument('-d', '--direction', dest='direction', choices=['in', 'out', 'both'], action='store',
                        default="both", help='filename of the store file')
    ...
    if args.list is True:
        ...
        direction = None if args.direction == 'both' else MessageDirection.INBOUND \
            if args.direction == "in" else MessageDirection.OUTBOUND

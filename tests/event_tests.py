...
class EventTimerTests(unittest.TestCase):
    def testTimerEvent(self):
        ...
        t1 = TimerEventRegistration(lambda fire, closure:
                                    self.assertEqual(int((datetime.datetime.utcnow() - closure).total_seconds()), 1),
                                    1.0,
    ...
    def testTimerEventReset(self):
        mgr = EventManager()
        t1 = TimerEventRegistration(lambda fire, closure: self.assertEqual(int((datetime.datetime.utcnow() - closure).
                                                                               total_seconds()), 2),
                                    1.0,
                                    datetime.datetime.utcnow())
        mgr.registerHandler(t1)
